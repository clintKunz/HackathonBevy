...
class SolicitPage extends React.Component {
  ...
  componentDidMount(){
    ...
    .then(res => {
      this.setState({solicit : res.data.loan[0]})
  ...
  render() {
    ...
    return (
      <SolicitContainer>
          ...
          <p>I hereby solicit a <span>{this.state.solicit.loanType}</span> loan in the amount of <span>${this.state.solicit.amount}</span>. With the following terms,</p>
          <p>The loan will be funded by the lender no later than two weeks after both parties have signed the contract. Payback for the loan will then commence on the day the contract is executed or <span>{moment(this.state.solicit.startDate).format('MMM-DD-YYYY')}</span>, whichever is later. Payments will be made monthly for the entire length of the loan, which is <span>{this.state.solicit.lengthMonths}</span> months. Monthly payments will be <span>${Math.round((this.state.solicit.amount/this.state.solicit.lengthMonths)*(1 + this.state.solicit.interest/100/12))}</span></p>
